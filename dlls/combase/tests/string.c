...
static void test_substring(void)
{
    ...
    check_string(newStr, output_substring, 4, FALSE);
    ...
    check_string(newStr, input_string, 6, FALSE);
    ok(pWindowsDeleteString(newStr) == S_OK, "Failed to delete string\n");
    ok(pWindowsDeleteString(str) == S_OK, "Failed to delete string\n");
    ...
    ok(pWindowsSubstring(NULL, 0, &newStr) == S_OK, "Failed to duplicate NULL string\n");
